...
function getVisibleHazardLanes() {
  return getBlockedLanesInWindow(0.03, OBJECT_DESPAWN_Z);
}
...
function getAvailableHazardLanes() {
  const visibleHazardLanes = getVisibleHazardLanes();
  if (visibleHazardLanes.size >= 2) {
    return [];
  }

  const blockedLanes = getBlockedLanesInWindow(0.03, 0.72);
  return [0, 1, 2]
    .filter((lane) => !visibleHazardLanes.has(lane))
    .filter((lane) => !blockedLanes.has(lane))
    .filter((lane) => !hasHazardTooCloseInLane(lane));
}

function getLanePoolForType(type) {
  if (isBlockingType(type)) {
    return getAvailableHazardLanes();
  }
  return getPickupLanePool();
}

function spawnHazard() {
  const lanePool = getLanePoolForType("cone");
  if (!lanePool.length) {
    return false;
  }
...
function spawnObject(type = null) {
  if (type && obstacleTypes[type]) {
    const lanePool = getLanePoolForType(type);
    if (!lanePool.length) {
      return false;
    }
    createObject(type, randomItem(lanePool));
    return true;
  }
